...
class JsonStreamingParser_Parser {
    ...
    protected function _consume_char($c) {
    ...
    private function _start_value($c) {
        if ($c === '[') {
            ...
            throw new JsonStreamingParser_ParsingError($this->_line_number, $this->_char_number, "Unexpected character for value: " . $c);
        }
    }
    ...
    private function _end_object() {
        ...
        if ($popped !== self::STACK_OBJECT) {
            throw new JsonStreamingParser_ParsingError($this->_line_number, $this->_char_number, "Unexpected end of object encountered.");
        }
    ...
    private function _process_escape_character($c) {
        if ($c === '"') {
            ...
            throw new JsonStreamingParser_ParsingError($this->_line_number, $this->_char_number, "Expected escaped character after backslash. Got: " . $c);
        }
    ...
    private function _process_unicode_character($c) {
        if (!$this->_is_hex_character($c)) {
            throw new JsonStreamingParser_ParsingError($this->_line_number, $this->_char_number, "Expected hex character for escaped unicode character. Unicode parsed: " . implode($this->_unicode_buffer) . " and got: " . $c);
        }
        ...
        if (count($this->_unicode_buffer) === 4) {
            ...
            if ($codepoint >= 0xD800 && $codepoint < 0xDC00) {
                ...
                if ($this->_unicode_high_codepoint === -1) {
                    throw new JsonStreamingParser_ParsingError($this->_line_number, $this->_char_number, "Missing high codepoint for unicode low codepoint.");
                }
    ...
    private function _end_true() {
        ...
        if ($true === 'true') {
            ...
            throw new JsonStreamingParser_ParsingError($this->_line_number, $this->_char_number, "Expected 'true'. Got: " . $true);
        }
        $this->_buffer = '';
        $this->_state = self::STATE_AFTER_VALUE;
    }
    ...
    private function _end_false() {
        ...
        if ($false === 'false') {
            ...
            throw new JsonStreamingParser_ParsingError($this->_line_number, $this->_char_number, "Expected 'false'. Got: " . $false);
        }
        $this->_buffer = '';
        $this->_state = self::STATE_AFTER_VALUE;
    }
